...
int main(int argc, char *argv[]){
    ...
    struct timespec start, end, packetStart, packetCurr;
    ...
    for(int i=0; i<totalFrag; i++){
        ...
        do{
            ...
            if(ifTimeout == 1){
                printf("Did not receive acknowledgment. Retransmitting packet %d \n", i);
                printf("ACK for package %d is %s not %s\n",i, corr_ack_buffer, resposeBuf);
                ...
                //update start first transmission time
                clock_gettime(CLOCK_MONOTONIC, &packetStart);
                ifTimeout = 0; //reset flag
            }
